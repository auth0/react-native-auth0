...
export default class UsersAPI {
  ...
  link(id, otherUserToken) {
    return Promise.all([
      nonNull(id, 'must supply an identifier of the user to link to'),
      nonNull(otherUserToken, 'must supply a valid token of the user to link')
  ...
  unlink(id, identityId, provider) {
    return Promise.all([
      nonNull(id, 'must provider an user identifier from where the identity will be unlinked'),
