...
describe('id token verification tests', () => {
  ...
  describe('token claims verification', () => {
    ...
    it('fails when "exp" is invalid outside of default leeway', async () => {
      const testJwt =
    ...
    it('succeeds when "exp" is in the past but within the default leeway', async () => {
      ...
      await expect(verify(credentials, overrides)).resolves.toEqual(
        credentials,
      );
    });

    it('fails when "exp" is invalid outside of custom leeway', async () => {
      const testJwt =
        'eyJhbGciOiJSUzI1NiJ9..uDn-4wtiigGddUw2kis_QyfDE3w75rWvu9NolMgD3b7l4_fedhQOk-z_mYID588ZXpnpLRKKiD5I2IFsXl7Qcc10rx1LIZxNqdzyc3VrgFf677x7fFZ4guR2WalH-zdJEluruMRdCIFQczIjXnGKPHGQ8gPH1LRozv43dl-bO2viX6MU4pTgNq3GIsU4ureyHrx1o9JSqF4b_RzuYvVWVVX7ABC2csMJP_ocVbEIQjUBhp1V7VcQY-Zgq0prk_HvY13g8FxK4KvSza637ZWAfonn599SKuy22PeMJqDfd64SbunWrt-mKBz9PHeAo9t4LJPLsAqSd3IQ2aJTsnqJRA';
      const credentials = getCredentials(testJwt);
    ...
    it('fails when "iat" is invalid outside of default leeway', async () => {
      const testJwt =
    ...
    it('fails when "iat" is invalid outside of custom leeway', async () => {
      const testJwt =
        'eyJhbGciOiJSUzI1NiJ9..KQWivvmcmWt5a3maRLFcMY8ev-gdeae-3Hm50BilmHjL31__S5G8dbHUOIjMg0zPIVbuml2SjXxEkOSiC-NZcupo7tknXuy76NdsMXWn1z7Cz1kI2XGFaR_PtY1lqpHc5FkQf1KiGhq4tMSk0RQDDu1U0E7WTDikV2mSrejOumL9qhj1lFwzCAr3ElDSnkHfcTuFQRMD0AdKFGD5oXOh4MgMFIE7GNFVAGnJ1Ld9JDbl1nqWTdrXZ3hYVDJDOc4DG8PdriFW3boyQKWqmV7eQZORKUW5C94VjcywZ9VROHqKyZXB6zc7s3FJ0zY2LxXEmTgOMEUgM2NtZVyMAhn0pQ';
      const credentials = getCredentials(testJwt);
    ...
    it('fails when "max_age" was sent on the authentication request but "auth_time" is invalid within default leeway', async () => {
      const testJwt =
    ...
    it('fails when "max_age" was sent on the authentication request but "auth_time" is invalid within custom leeway', async () => {
      const testJwt =
        'eyJhbGciOiJSUzI1NiJ9..AbSYZ_Tu0-ZelCRPuu9jOd9y1M19yIlk8bjSQDVVgAekRZLdRA_T_gi_JeWyFysKZVpRcHC1YJhTH4YH8CCMRTwviq3woIsLmdUecjydyZkHcUlhHXj2DbC15cyELalPNe3T9eZ4ySwk9qRJSOkjBAgXAT0a7M6rwri6QHnL0WxTLX4us4rGu8Ui3kuf1WaZH9DNoeWYs1N3xUnOwTkRKaqXnuKjnwSVmsuwxFSlnIPJOiMUUZksiaBq_OUvOkB-dEG7OFiDX9XWj1m62yBHkvZHun8LBr9VW3mt1IrcBdbbtzjWwfn6ioK2c4dbtPFhuYohXsmRDaSekP63Dmlw3A';
      const credentials = getCredentials(testJwt);
