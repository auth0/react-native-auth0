...
| `INVALID_CALLBACK_URL`       | The URL that returned to the app did not match the expected redirect. On iOS this commonly happens when an in-page link (e.g. a privacy-policy link that uses the app's custom scheme) is captured by `ASWebAuthenticationSession` instead of the real callback. Prefer HTTPS/Universal Link redirects to avoid this. | -                               | `INVALID_CALLBACK_URL`          |                                      |
